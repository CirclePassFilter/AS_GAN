...
def make_dataset(dataset, dataroot, imageSize):
    ...
    if dataset in ['imagenet', 'folder', 'lfw']:
        ...
        dataset = dset.LSUN(root=dataroot, classes=['bedroom_train'],  #db_path
                            transform=transforms.Compose([
                                transforms.Resize(imageSize),
                                transforms.CenterCrop(imageSize),
                                transforms.ToTensor(),
...
def sampleFake(netG, nz, sampleSize, batchSize, saveFolder, epoch):
    print('sampling fake images of {} ...'.format(epoch))
...
def sampleTrue(dataset, imageSize, dataroot, sampleSize, batchSize, saveFolder, epoch, workers=4):
    print('sampling real images of {}...' .format(epoch))
    saveFolder = saveFolder + '0/'  #.format(epoch, epoch)
...
def compute_score_raw(dataset, imageSize, dataroot, sampleSize, batchSize,
                      saveFolder_r, saveFolder_f, netG, nz, epoch,
                      conv_model='inception_v3', workers=4):

    sampleTrue(dataset, imageSize, dataroot, sampleSize, batchSize,
               saveFolder_r, epoch, workers=workers)

    sampleFake(netG, nz, sampleSize, batchSize, saveFolder_f, epoch)
    ...
    fid_value = Fid.calculate_fid_given_paths(paths=(saveFolder_r + '0/', saveFolder_f + '0/'), batch_size=60, cuda=True, dims=2048)
    is_mean, is_std = IS.calculate_inception_score(path= saveFolder_f + '0/')
    # for i in range(0, 4):
    ...
    # score[28] = inception_score(feature_f[2])
    # score[29] = mode_score(feature_r[2], feature_f[2])
    # score[30] = fid(feature_r[2], feature_f[2])
